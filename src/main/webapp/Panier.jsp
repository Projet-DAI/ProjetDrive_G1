	...
	<div id="page-content" class="page-content">
		...
		<section id="cart">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<button id="voirPointsFidelitebtn" class="btn btn-primary">Débloquer
									...
									<tr>
										...
										<td>
											<form action="ModifierQuantitePanierServlet" method="post">
												...
												<input type="number"
													class="vertical-spin form-control input-number quantity-input"
													value="<%=lignePanier.getQuantite()%>" min="1">
												<button type="button"
													class="btn btn-primary modify-quantity">Modifier</button>
											</form>
										</td>
										<script>
										...
										<td><a href="#" class="text-danger delete-item"
											data-id="<%=lignePanier.getProduit().getIdProduit()%>"><i
    ...
    var totalPanierString = '<%=String.valueOf(total)%>
							...
							function calculerNouveauTotal(pointsFidelite) {
								...
								if (!isNaN(totalPanier)) {
									...
									console
											.error("Erreur de conversion du total en nombre.");
								}
							}

							// Code existant pour l'événement 'voirPointsFidelitebtn'
							document
									.getElementById('voirPointsFidelitebtn')
									.addEventListener(
											'click',
											function() {
												var pointsFidelite =
						<%=new ClientDAO().getPointsFideliteById(1)%>
							;
						...
						<h6 class="mt-3">
							Total après réduction : <span id="nouveauTotalPanier">&#8364</span>
						</h6>
						<form id="checkoutForm" action="ChoixCreneauServlet" method="POST">
							<input type="hidden" id="nouveauTotal" name="nouveauTotal"
								value="">
