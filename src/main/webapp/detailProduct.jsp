...
<body>
	...
	<div id="page-content" class="page-content">
		...
		<div class="product-detail">
			<div class="container">
				<p>
					<a href="shop"><strong><i class="bi bi-arrow-left"></i>
							Retour</strong></a>
				</p>
				<!--    <a href="">
				...
				<div class="row">
					<div class="col-sm-6">
						...
						<img src="${produit.adresseImageProduit}" alt="Image du produit"
							style="width: 60%; text-align: right;">
					</div>
					<div class="col-sm-6">
						<h3>${produit.nomProduit}(${produit.marqueProduit})</h3>
						<p>
							<strong></strong> ${produit.kiloProduit}
						</p>
						<p>
							<strong>Prix:</strong>
							<%
							if (p.isPromotion()) {
							%><%=new java.text.DecimalFormat("#,###.00").format(p.getPrixProduit() * p.getPourcentagePromotion())%>
							<%
							} else {
							%>
							<%=p.getPrixProduit()%>
						...
						<p>
							<strong>Nutriscore:</strong> ${produit.nutriscore}
						</p>
						<p id="descriptionText">
							<strong>Description:</strong> ${not empty produit.description ? produit.description.substring(0, produit.description.length() > 100 ? 100 : produit.description.length()) : ''}
						</p>
	...
	<script>
		...
		function verifierQuantite() {
			...
			var quantiteDemandee = parseInt(document
					.getElementById("quantiteInput").value);

			// Faire une requête Ajax pour récupérer la quantité en stock
			var xhr = new XMLHttpRequest();
			xhr.open("GET",
					"GetQuantiteEnStock?idProduit=${produit.idProduit}", false);
			xhr.send();
